...
def schedule_item_metrics(cursor, metrics):
    cursor.execute("SELECT S.start_time,S.runtime,C.name,C.account_name,S.feed_id,S.tenant_id FROM schedule_item S INNER JOIN cp_and_ms C ON C.cloudport_feed_id = S.feed_id WHERE start_time BETWEEN (now() - interval '1 day') AND (now() + interval '30 day') ORDER BY S.start_time")
